...
Lopaka Lee. Available on \textsc{CRAN} \url{http://cran.r-project.org/}}, but
...
\section{Finding function names}
...
?glp_add_cols
@
will bring up the man-page of \texttt{addColsGLPK}. Keep in mind that some of
